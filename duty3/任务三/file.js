...
http.createServer(function(req,res){
    ...
    if(pathName=="/list"){
        ...
        var chapterLists=JSON.stringify(chapterList);
        res.writeHead(200,{"Content-Type":"text/plain"});
        res.write(chapterLists);
        res.end();
    }
        ...
        req.on("end",function(){
            var userObj=querystring.parse(userStr);
            for(var i=0;i<userList.length;i++){
                if(userObj.username==userList[i].username && userObj.pwd==userList[i].pwd){
        ...
        req.on("end",function(){
            var addObj=querystring.parse(addStr);
            var listObj={};
            if(addObj.title!="" && addObj.content!=""){
                listObj["chapterId"]=chapterList.length+1;
                listObj["chapterName"]=addObj.title;
                listObj["chapterContent"]=addObj.content;
                listObj["views"]=200;
                chapterList.push(listObj);
                // localStorage.setItem("key",JSON.stringify(chapterList));
            }
            res.end();
        })
    }
    ...
    else if(pathName=="/favicon.ico" || pathName=="/bg.jpg" || pathName=="/"){
        res.end();
    }
        ...
        // console.log(showHtml(res,pathName));
    }
    else if(pathName.indexOf(".jpg")>=0 || pathName.indexOf(".jpeg")>=0 || pathName.indexOf(".png")){
